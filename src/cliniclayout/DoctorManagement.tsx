...
import {
  ...
  Loader2, Eye, Edit2, CheckCircle, AlertTriangle
} from "lucide-react";
...
const DoctorManagement: React.FC = () => {
  ...
  const mappedDoctors = doctors.map((doc: any) => ({
    ...
    isStripeVerified: doc.isStripeAccountVerified,
    user: doc.user,
    bio: doc.bio,
    docRaw: doc
  }));
            ...
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead className="bg-slate-50/80 border-b border-slate-100 text-[10px] font-black uppercase text-slate-400 tracking-widest">
                <tr>
                  ...
                  <th className="px-8 py-5">Stripe Verify</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {mappedDoctors.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-8 py-16 text-center text-slate-400 font-bold">No results found for your query.</td>
                  </tr>
                  ...
                  <tr key={doc.id} className="hover:bg-slate-50/80 transition-colors group">
                    ...
                    <td className="px-8 py-6">
                      <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase flex items-center gap-2 ${doc.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'
                        }`}>
                    ...
                    <td className="px-8 py-6">
                       {doc.isStripeVerified ? (
                          <span className="flex items-center gap-2 text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-100">
                             <CheckCircle size={14} /> Verified
                          </span>
                       ) : (
                          <span className="flex items-center gap-2 text-rose-600 bg-rose-50 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-rose-100">
